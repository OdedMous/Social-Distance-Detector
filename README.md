...
## We'll focus on phase 2 – How to calculate the distance between a pair of people?
...
1) The pair at the front of the scene were not classified as violating the distance restriction, even though they are close to each other (in contrast, note that the women and the baby-wagon were classified as close to each other as requierd).
...
1) <ins> There is no uniform distance threshold: </ins> In this frame, for two pairs of pepole that their real distance is the same, the distance in pixels they reflecting is not necessarily the same. This is because as we get closer to the infinity point in the image, the distance in pixels between objects becomes smaller, while the real distnace may stay the same. Hence the single defined threshold that we used worked for pepole that appeared in the middle of the image (the woman and the baby-wagon), and not for pepole that appeared in the front of the image (the couple). To conclude - the distance in pixels between two people varies depending on whether they are in the front or in the background of the frame, and hence we can't define a single threshold that is suitable for every pair of people in the frame.
